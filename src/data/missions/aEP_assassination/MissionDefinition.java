package data.missions.aEP_assassination;
...
public class MissionDefinition implements MissionDefinitionPlugin
{
  ...
  public void defineMission(final MissionDefinitionAPI api) {
    ...
    api.setFleetTagline(FleetSide.PLAYER, txt("aEP_Mission06_01"));
    api.setFleetTagline(FleetSide.ENEMY, txt("aEP_Mission06_02"));

    // These show up as items in the bulleted list under
    // "Tactical Objectives" on the mission detail screen
    String name1 = txt("aEP_Mission06_name1");

    //在这加自己的船，用装配文件的ID，后面是船名，true和false是“是否是旗舰”的设定
    // Set up the player's fleet.  Variant names come from the
    // files in data/variants and data/variants/fighters
    api.addToFleet(FleetSide.PLAYER, "aEP_fga_shuangshen_Standard", FleetMemberType.SHIP, name1, true);



    //在这加敌人的船，一样用装配文件里的ID，加了一艘统治者的Support装配
    // Set up the enemy fleet.
    api.addToFleet(FleetSide.ENEMY, "aEP_conquest_Standard", FleetMemberType.SHIP,  false);
    //设置输赢条件
